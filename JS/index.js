...
$(function(){  
	...
	var bBH = new Array();
	var banner;
	var bannerBgColor = ["#FFDEAD","#7CDCD8","#F0E68C","#F5F5F5","#7CFC00"]; 
	var bannerImg = ["Images/校门.jpg","Images/图书馆.jpg","Images/三彩艺术馆.jpg","Images/雪景.jpg","Images/小绿.jpg"];//定义左边的图片
	...
	function changWallPaper(btnIndex){
			...
			switch(bannerNowIndex){
				...
				case 2:
					createBanner(2);
					$(".bannerImage").animate({right:"0px",opacity:1},chageTime(),chageState());
	...
	if(window.browserIE6){
		// $(".bannerInner , .bannerLeftBar").css({display:"none"});
		...
		// 	bannerAutoPlayHolder = setInterval(bannerAutoPlay,6000);
		// });
		 $(".bannerLeftBar ").css({opacity:0});
			$(".Banner").hover(function(){
			   $(".bannerLeftBar ").animate({opacity:1},200).dequeue();
		 },function(){
			   $(".bannerLeftBar ").animate({opacity:0},200).dequeue();
		 })
	}
